    ...
    public static void Go()
    {
        ...
        while (true)
        {
            ...
            else
            {
                ...
                // Message to send
                string text_to_answer = text;

                // Send back the same message
                byte[] response = new byte[100 + text_to_answer.Length];
                ...
                if (msglen < 126)
                {
                    response[1] |= (byte)text_to_answer.Length;
                    starts_writing_payload_at = 2;
                }
                else if (msglen < 65536)
                {
                    response[1] |= 126;
                    byte[] len = BitConverter.GetBytes((ushort)text_to_answer.Length);
                    response[2] = len[1];
                    response[3] = len[0];
                ...
                {
                    response[1] |= 127;
                    byte[] len = BitConverter.GetBytes(text_to_answer.Length);
                    response[2] = len[7];
                    response[3] = len[6];
                    response[4] = len[5];
                    response[5] = len[4];
                ...
                for (ulong i = 0; i < (ulong)text_to_answer.Length; ++i)
